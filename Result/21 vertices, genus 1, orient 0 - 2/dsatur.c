...
print_colors() 
{
  ...
  //if(DEBUG) printf("Best coloring is %d at time %7.1f\n",BestColoring,(current_time-start_time)/60.0);
  
  for (i=0;i<num_node;i++)
		ColorClass_Saved[i] = ColorClass[i];
  ...
  char *argv[];
{
  ...
  if(DEBUG) printf("Graph %s read with %d nodes and %d edges\n",argv[1],num_node,m);
  for (i=0;i<num_node;i++)
    Handled[i] = FALSE;
...
/*  ColorClass[0] = 1;
  ...
  if(DEBUG)  printf("Lower bound is %d",lb);
  ...
  //current_time=buffer.tms_utime;
	...
	if(DEBUG) 
		for (i=0;i<num_node;i++)
			printf("Color[%3d] = %d\n",i,ColorClass_Saved[i]);
	
	for (i=0;i<num_node;i++)
	{
		printf("%d",ColorClass_Saved[i]);
		if(i<(num_node-1))
			printf(",");
	}
  ...
  //if(DEBUG) printf("Best coloring has value %d, subproblems: %d time:%7.1f\n",val,prob_count,(current_time-start_time)/60.0);
